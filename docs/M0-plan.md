...
| Routing | **No dependency** — ~40-line hash router in `src/lib/hashRouter.ts` | react-router is ~10KB + large API for ~8 views. Hash routing also means the built app works from `file://` and any static host with zero rewrite rules. *(M7 correction: the `file://` half of this was wrong — module scripts are CORS-blocked from a file path. Any static host, root or subdirectory, is right.)* |
